/**
 * @class Network
 * @brief Manages a network of routers, providing functionalities for network topology and routing.
 *
 * The Network class encapsulates a collection of routers and their connections, simulating a network environment.
 * It allows for the initialization of the network with a set of routers, the establishment of connections between routers,
 * and the calculation of routing paths using Dijkstra's algorithm. The class supports dynamic network configurations,
 * enabling the addition of new connections and the recalibration of routing paths as the network evolves.
 */
#pragma once
...
public:
    ...
    /**
     * @brief Destructor. Traverses the list of routers, deleting each router.
     */
    ~Network();
    /**
